...
func (s *ConsulSourceTestSuite) SetupSuite() {
	...
	// Set the Consul HTTP address environment variable to allow Synthra to connect to the Consul server
...
func (s *ConsulSourceTestSuite) TestLoad_ValuePresent() {
	...
	consul, err := NewConsul(key, codec.JSON, nil)
	s.Require().NoError(err)
...
func (s *ConsulSourceTestSuite) TestLoad_ValueAbsent() {
	...
	consul, err := NewConsul("test/value-absent", codec.JSON, nil)
	s.Require().NoError(err)
...
`
	...
	// Create Consul source
	consul, err := NewConsul(key, codec.YAML, nil)
	s.Require().NoError(err)
...
// TestLoad_WithParseFloat64 tests the Load method with a ParseFloat64
// scalar decoder
func (s *ConsulSourceTestSuite) TestLoad_WithParseFloat64() {
	// Set up test data
	key := "test/caster"
	value := `42`
	_, err := s.client.KV().Put(&api.KVPair{
	...
	// Create Consul source with scalar decoder
	consul, err := NewConsul(key, codec.ParseFloat64("caster"), nil)
	s.Require().NoError(err)

	// Load configuration
	conf, err := consul.Load(context.Background())
	s.Require().NoError(err)
	v, ok := conf["caster"].(float64)
	s.Require().True(ok)
	s.InDelta(float64(42), v, 0.001)
}
...
func (s *ConsulSourceTestSuite) TestLoad_WithContextTimeout() {
	...
	consul, err := NewConsul("test/timeout", codec.JSON, mockKV)
	s.Require().NoError(err)
...
func (s *ConsulSourceTestSuite) TestLoad_WithNonExistentKey() {
	// Create Consul source with non-existent key
	consul, err := NewConsul("non/existent/key", codec.JSON, nil)
	s.Require().NoError(err)
...
// TestLoad_WithClientInitFailure tests the Load method with a client
...
func (s *ConsulSourceTestSuite) TestLoad_WithClientInitFailure() {
	...
	// Create Consul source with mock KV
	consul, err := NewConsul("test/key", codec.JSON, mockKV)
	s.Require().NoError(err)
...
// TestLoad_WithKVOperationFailure tests the Load method with a KV
// operation failure
...
// TestLoad_WithSpecialCharacters tests the Load method with special
...
func (s *ConsulSourceTestSuite) TestLoad_WithBinaryValue() {
	...
	// Create Consul source with string scalar decoder
	consul, err := NewConsul(key, codec.ParseString("binary-value"), nil)
	s.Require().NoError(err)

	// Load configuration
	conf, err := consul.Load(context.Background())
	s.Require().NoError(err)
	// ParseString trims whitespace; binary bytes with no surrounding whitespace should be preserved
...
// TestLoad_WithInvalidParseIntValue tests the Load method with an invalid
// value for ParseInt
func (s *ConsulSourceTestSuite) TestLoad_WithInvalidParseIntValue() {
	// Set up test data with invalid value for int parsing
	...
	// Create Consul source with scalar int decoder
	consul, err := NewConsul(key, codec.ParseInt("test"), nil)
	s.Require().NoError(err)
