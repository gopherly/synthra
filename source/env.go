...
// OSEnvVar represents a configuration source that loads data from
// environment variables.
// It filters environment variables by prefix and creates nested
// configuration structures based on underscore-separated variable names.
//
// For example, with prefix "APP_", the environment variable
// "APP_SERVER_PORT" becomes the configuration key "server.port".
...
func NewOSEnvVar(prefix string) *OSEnvVar {
	return &OSEnvVar{
		prefix:  prefix,
		decoder: codec.EnvVar,
	}
}

// Load reads environment variables with the configured prefix and
// decodes them into a map[string]any.
// Variable names are converted to lowercase and underscores create
// nested structures.
//
// Example:
//
