...
func BenchmarkGetString(b *testing.B) {
	...
	for b.Loop() {
		_, _ = c.String("foo") //nolint:errcheck // benchmark: hot path
	}
}

// BenchmarkGetInt benchmarks integer type conversion.
func BenchmarkGetInt(b *testing.B) {
	src := source.NewMap(map[string]any{"num": 42})
	c, err := New(WithSource(src))
	if err != nil {
		b.Fatal(err)
	}
...
func BenchmarkGetStringSlice(b *testing.B) {
	...
	for b.Loop() {
		_, _ = c.StringSlice("tags") //nolint:errcheck // benchmark: hot path
	}
}
...
func BenchmarkLargeConfig(b *testing.B) {
	...
	for b.Loop() {
		_, _ = c.String("key500") //nolint:errcheck // benchmark: hot path
	}
}
...
func BenchmarkMultipleSources(b *testing.B) {
	sources := []Source{
		source.NewMap(map[string]any{"key1": "value1"}),
		source.NewMap(map[string]any{"key2": "value2"}),
		source.NewMap(map[string]any{"key3": "value3"}),
	}
...
func BenchmarkValidator(b *testing.B) {
	src := source.NewMap(map[string]any{"port": 8080})
